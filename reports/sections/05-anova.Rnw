...
Looking at the earnings gap metric amongst different student income groups, we see that the outcome gap is significant when comparing with low income students for all institution types. The gap is significant between Private Non-Profit and Public schools for high and mid income students.
